...
export default {
  ...
  mounted() {
    ...
    c.width = document.body.clientWidth - 10
    c.height = document.body.clientHeight - 15
    const m = Math
    const C = Math.cos
    const S = Math.sin
    const P = Math.pow
    ...
    const p = function(a, b, c) {
      if (c > 60) {
        return [S(a * 7) * (13 + 5 / (0.2 + P(b * 4, 4))) - S(b) * 50, b * f + 50, 625 + C(a * 7) * (13 + 5 / (0.2 + P(b * 4, 4))) + b * 400, a * 1 - b / 2, a]
      }
      const A = a * 2 - 1
      const B = b * 2 - 1
      if (A * A + B * B < 1) {
